...
widget.FilePicker.prototype =
{
     ...
     *
     */
    submitFormUsingAjax : function( theForm, errorMsg, sendBeacon, afterSubmitCallback )
    {
      ...
      page.util.setProgressBarVisible( true );
      $j.ajax(
      {
          url : theForm.action,
          data : data,
          ...
          success : function( response )
          {
            page.util.setProgressBarVisible( false );
            ...
            {
              afterSubmitCallback( true, response );
            }
          }.bind( this ),
          error : function( data )
          {
            ...
            if( afterSubmitCallback )
            {
              afterSubmitCallback( false, data );
            }
          }
      } );
    },
    ...
    onLocalPick : function( event )
    {
      if ( this.localFilePicker.value != '' || event && event.dataTransfer && event.dataTransfer.files && event.dataTransfer.files.length > 0 )
      {
        ...
        else
        {
          ...
          // Escaping file names for preventing XSS
          var localFileName = ( this.getFileName( this.localFilePicker.value ) ).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
